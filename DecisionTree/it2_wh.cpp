    ...
    int dfs(vector<string> &row, int here)
{
    if (tree[here].isLeaf)
    {
        return here;  // Base case: reached leaf node
    }

    int criteriaAttrIndex = tree[here].criteriaAttrIndex;

    // Iterate through the children and recurse if the row matches the criteria
    for (int i = 0; i < tree[here].children.size(); i++)
    {
        ...
        {
            return dfs(row, next);  // Match found, recurse into child
        }
    }

    // If no match is found, blindly go to the first child
    if (!tree[here].children.empty())
    {
        return dfs(row, tree[here].children[0]);  // Go to the first child if no match found
    }

    return -1;  // Return -1 if no valid path was found (shouldn't reach here unless tree is malformed)
}
...
int main()
{
    ...
    ofstream kf_statsFile("k_fold_test_stats_wh.txt");
    int k;
