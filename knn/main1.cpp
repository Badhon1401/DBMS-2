...
void queryMinDistance(KDTree &tree, size_t dimensions) {
    ...
    try {
        ...
        double minDistance = numeric_limits<double>::max();
        vector<double> nearestPoint=tree.calculateDistancesRecursive(tree.getRoot(), queryPoint, 0, minDistance);
...
int main() {
   ...
   // Training Phase: Process the query points and compute their minimum distance
