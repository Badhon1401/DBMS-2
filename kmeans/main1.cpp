...
vector<Point> readBMP(const string &filename, int &width, int &height, int &dimensions) {
    ifstream file(filename, ios::binary);
    if (!file.is_open()) {
        cerr << "Error: Could not open BMP file!" << endl;
        exit(1);
    }
    ...
    if (header[28] != 24 || header[30] != 0) {
        cerr << "Error: Only 24-bit uncompressed BMP files are supported!" << endl;
        exit(1);
    }

    // Move to pixel data offset
    file.seekg(pixelDataOffset, ios::beg);

    // Read pixel data (BGR format, padded to 4-byte rows)
    vector<Point> points;
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            string line = to_string(bgr[2]) + " " + to_string(bgr[1]) + " " + to_string(bgr[0]); // RGB order
            points.emplace_back(pointId++, line);
        }
