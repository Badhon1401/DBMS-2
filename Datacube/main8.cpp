...
vector<pair<string, AggregatedValues>> generateRollup(const vector<DataRow> &data, const vector<string> &columnNames, const string &finalColumnName)
{
    ...
    // Step 2: Perform intermediate rollup aggregations, avoiding over-counting
    for (size_t i = 0; i < columnNames.size() - 1; ++i)
    {
        ...
        for (const auto &row : data)
        {
            ...
            {
                // Use "NULL" for higher-level groupings
    ...
    // Grand total should aggregate only once across all data
    ...
    rollupResults["NULL, NULL"] = grandTotalValues;
...
vector<pair<string, AggregatedValues>> generateCube(const vector<DataRow> &data, const vector<string> &columnNames, const string &finalColumnName)
{
    ...
    // Step 3: Add grand total (with all NULL)
    string grandTotalKey = string(numColumns - 1, 'N') + ", "; // Placeholder for all levels
    grandTotalKey.pop_back();
    AggregatedValues grandTotalValues;
    grandTotalValues.addValue(grandTotal);
    cubeResults[grandTotalKey] = grandTotalValues;
