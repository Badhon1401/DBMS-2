...
void printAggregationResults(const map<string, AggregatedValues>& aggregationResults, const vector<string>& columnNames, const vector<int>& selectedAggregations, const vector<int>& selectedColumns, const string& tableName, const string& finalColumnName) {
    size_t aggColWidth = 10;
    ...
    if (findAggregationType(selectedAggregations, 1)) sqlQuery << "SUM(" << finalColumnName << ") AS SUM, ";
    if (findAggregationType(selectedAggregations, 2)) sqlQuery << "COUNT(" << finalColumnName << ") AS COUNT, ";
    if (findAggregationType(selectedAggregations, 3)) sqlQuery << "MIN(" << finalColumnName << ") AS MIN, ";
    if (findAggregationType(selectedAggregations, 4)) sqlQuery << "MAX(" << finalColumnName << ") AS MAX, ";
    if (findAggregationType(selectedAggregations, 5)) sqlQuery << "AVG(" << finalColumnName << ") AS AVG, ";
    ...
    if (!query.empty() && query[query.size() - 2] == ',') {
        query = query.substr(0, query.size() - 2); // Remove trailing comma
    }
...
void interactiveAggregationQuery(const map<string, AggregatedValues>& aggregationData, const vector<string>& columnNames, const string& tableName, const string& finalColumnName) {
    while (true) {
        system("cls");
        ...
        printAggregationResults(aggregationData, columnNames, aggregationTypes, selectedColumns, tableName, finalColumnName);
...
int main() {
    ...
    interactiveAggregationQuery(cubeMap, columnNames, tableName, finalColumnName);
