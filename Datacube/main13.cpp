...
void printAggregationResults(const map<string, AggregatedValues>& aggregationResults, const vector<string>& columnNames, const vector<int>& selectedAggregations, const vector<int>& selectedColumns) {
    ...
    // Ensure printedRows is declared inside the function
    set<string> printedRows;

    // Print column names for selected columns
    for (int colIndex : selectedColumns) {
        cout << left << setw(aggColWidth) << columnNames[colIndex];
    }

    // Print aggregation headers
    ...
    // Iterate through the aggregation results and print the rows
    for (const auto& entry : aggregationResults) {
        ...
        // Check if the row has already been printed
        if (printedRows.find(rowStr) == printedRows.end()) {
            ...
            const AggregatedValues& agg = entry.second;
            // Print aggregation values based on the selected aggregations
...
void interactiveAggregationQuery(const map<string, AggregatedValues>& aggregationData, const vector<string>& columnNames) {
    while (true) {
        ...
        cout << "Please enter the columns you want to query by (e.g., 1,2,4 or type 'exit' to quit, 0 for all columns): ";
        string columnInput;
...
int main()
{
    ...
    interactiveAggregationQuery(rollupMap, columnNames);
