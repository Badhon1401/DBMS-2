...
bool findAggregationType(const set<int> &selectedAggregations, int type)
{
    for (int aggType : selectedAggregations)
    {
        if (aggType == type)
            return true;
    }
...
void printAggregationResults(const map<string, AggregatedValues> &aggregationResults, const vector<string> &columnNames, const set<int> &selectedAggregations, const set<int> &selectedColumns, const string &tableName, const string &finalColumnName, const bool isRollup, const bool isnormalGroupBy)
{
    size_t aggColWidth = 10;
    ...
    for (const auto &entry : aggregationResults)
    {
        ...
        if (!isnormalGroupBy)
        {
            for (size_t i = 0; i < values.size(); ++i)
            {
                if (selectedColumns.find(static_cast<int>(i)) == selectedColumns.end() && values[i] != "NULL")
                {
                    isValidRow = false;
                    break;
                }
...
void interactiveAggregationQuery(const map<string, AggregatedValues> &cubeMap, const map<string, AggregatedValues> &rollUpMap, const vector<string> &columnNames, const string &tableName, const string &finalColumnName)
{
    ...
    {
        system("cls");
        cout << "Welcome to the ROLL UP and CUBE operator based Aggregation Query system!\n";
        ...
        cout << "Please enter the columns you want to query by (e.g., 0,1,2 or type -1 to quit): ";
        string columnInput;
        getline(cin, columnInput);

        if (columnInput.find("-1") != string::npos)
        {
        ...
        while (getline(columnStream, colStr, ','))
        {
            try
            {
                ...
                {
                    selectedColumns.insert(colIndex);
                }
                else
                {
        ...
        while (true)
        {
            ...
            cout << "1. SUM\n2. COUNT\n3. MIN\n4. MAX\n5. AVG\n";
            cout << "Enter the numbers of the aggregation types (e.g., 1,3,5 or type -1 to quit): ";
            string aggregationChoice;
            getline(cin, aggregationChoice);

            if (aggregationChoice.find("-1") != string::npos)
            {
                cout << "Exiting the query system.\n";
                return;
            }
            ...
            while (getline(aggStream, aggType, ','))
            {
                try
                {
                    ...
                    if (agg >= 1 && agg <= 5)
                    {
                        selectedAggregations.insert(agg);
                    }
                    else
                    {
                        cout << "Invalid aggregation type: " << agg << ". Valid options are 1-5.\n";
                        cin.get();
                        selectedAggregations.clear();
                        break;
                    }
                }
                catch (...)
            ...
            if (!selectedAggregations.empty())
            {
                break;
            }
        }

        bool isNormalGroupBy = false;
        while (true)
        {
            system("cls");
            cout << "Would you like to perform a normal GROUP BY (Y) or ROLL UP, CUBE based GROUP BY (N)? (or type -1 to quit): ";
            string groupByChoice;
            getline(cin, groupByChoice);
            ...
            if (groupByChoice == "Y" || groupByChoice == "y")
            {
                isNormalGroupBy = true;
                break;
            }
            else if (groupByChoice == "N" || groupByChoice == "n")
            {
                isNormalGroupBy = false;
                break;
            }
            else
            {
        ...
        if (((numOfsc * (numOfsc - 1)) / 2) == sum)
        {
            if (isNormalGroupBy)
            {
                printAggregationResults(rollUpMap, columnNames, selectedAggregations, selectedColumns, tableName, finalColumnName, isRollUp, isNormalGroupBy);
            }
            else
            {
                while (true)
                {
                    system("cls");
                    cout << "Would you like to perform a Rollup (R) or Cube (C) operation? Enter R or C (or type -1 to quit): ";
                    ...
                    if (operationChoice == "R" || operationChoice == "r" || operationChoice == "C" || operationChoice == "c")
                    {
                        isRollUp = (operationChoice == "R" || operationChoice == "r");
                        if (isRollUp)
                        {
                            printAggregationResults(rollUpMap, columnNames, selectedAggregations, selectedColumns, tableName, finalColumnName, isRollUp, isNormalGroupBy);
                        }
                        ...
                        {
                            printAggregationResults(cubeMap, columnNames, selectedAggregations, selectedColumns, tableName, finalColumnName, isRollUp, isNormalGroupBy);
                        }
                        break;
                    }
                    else
                    {
                        cout << "Invalid input! Please enter R for Rollup, C for Cube, or -1 to quit.\n";
                        cin.get();
                    }
                }
            }
...
int main()
{
    ...
    vector<DataRow> data = parseInputFile("input2.txt", tableName, columnNames, finalColumnName);
    ...
    interactiveAggregationQuery(cubeMap, rollupMap, columnNames, tableName, finalColumnName);
