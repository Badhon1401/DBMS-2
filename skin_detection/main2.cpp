...
private:
    ...
    // Use vector of ColorProbability instead of 3D matrix
    vector<ColorProbability> color_probabilities;
    ...
    void processBatch(const string& maskDir, const string& imageDir, 
                      int start, int end) {
    ...
    // Update color probabilities more efficiently
    void updateColorProbabilities(const vector<vector<Pixel>>& mask_pixels, 
        ...
        for (size_t y = 0; y < image_pixels.size(); ++y) {
            for (size_t x = 0; x < image_pixels[y].size(); ++x) {
                ...
                // Flatten color tuple to linear index
                size_t color_index = (image_pixel.r * 256 * 256) + 
                                     (image_pixel.g * 256) + 
                                     image_pixel.b;

                // Expand vector if needed
                if (color_probabilities.size() <= color_index) {
                    color_probabilities.resize(color_index + 1);
                }

                // White/near-white mask indicates non-skin
                if (mask_pixel.r > 250 && mask_pixel.g > 250 && mask_pixel.b > 250) {
                    color_probabilities[color_index].non_skin_count++;
                } else {
                    color_probabilities[color_index].skin_count++;
                }
            }
        }
    }
...
public:
    ...
    void detectSkin(const string& inputImage, 
        ...
        for (auto& row : image_pixels) {
            for (auto& pixel : row) {
                // Linear color index
                size_t color_index = (pixel.r * 256 * 256) + 
                                     (pixel.g * 256) + 
                                     pixel.b;

                // Check probability and apply skin detection
                if (color_index < color_probabilities.size() && 
                    color_probabilities[color_index].probability <= threshold) {
                    pixel.r = pixel.g = pixel.b = 255;  // White out non-skin
                }
            }
        }

        // Write processed image (simplified, you'd need actual BMP writing logic)
        ofstream output(outputImage, ios::binary);
        for (const auto& row : image_pixels) {
...
int main() {
    try {
        ...
        detector.processDataset("Mask", "ibtd", 555, 50);
