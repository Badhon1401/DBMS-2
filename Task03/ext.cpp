...
vector<set<int>> generateCandidates(const vector<set<int>> &prevItemsets)
{
    ...
    for (int i = 0; i < prevItemsets.size(); ++i)
    {
        ...
        {
            const auto &set1 = prevItemsets[i];
            const auto &set2 = prevItemsets[j];

            for (int elem : set2)
            {
                set<int> newCandidate = set1;
                newCandidate.insert(elem);

                if (newCandidate.size() == set1.size() + 1)
                {
                    uniqueCandidates.insert(move(newCandidate));
                }
            }
        }
    }
